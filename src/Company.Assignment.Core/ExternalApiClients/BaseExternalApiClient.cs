...
public abstract class BaseExternalApiClient(HttpClient httpClient, ILogger<BaseExternalApiClient> logger, JsonSerializerOptions jsonSerializerOptions)
{
    ...
    protected async Task<ApiResponse<T?>> GetRequest<T>(string requestUri, Dictionary<string, StringValues>? queryParams = null, CancellationToken cancellationToken = default)
    {
